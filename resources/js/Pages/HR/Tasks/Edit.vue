<template>
  <HRLayout>
    <template #title>تعديل نموذج المهمة الميدانية (Task Builder - M3-P02)</template>

    <div class="space-y-6 max-w-5xl mx-auto">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-2xl font-black text-slate-800 dark:text-white">تعديل نموذج المهمة (Task Builder)</h2>
          <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">تحديث الخصائص والمواقع والاستشاريين والمكونات التفاعلية والخيارات</p>
        </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <!-- Step 3: Dynamic Form Components -->
        <SpatialCard title="3. بناء المكونات التفاعلية والمنطق الشرطي">
          <div class="space-y-6">
            <div
...
<script setup>
/**
 * Edit.vue - تعديل نموذج المهمة (Task Builder المطور)
 */
...
watch(() => props.task, (t) => {
  if (t) {
    ...
    form.is_required = t.is_required !== undefined ? Boolean(t.is_required) : true
    form.site_ids = (t.sites || []).map(s => s.id)
    form.consultant_ids = (t.consultants || []).map(c => c.id)
    form.components = (t.components || []).map(c => {
      ...
      return {
        ...
        options: (c.options || []).map(normalizeOptionText),
...
const removeOption = (comp, optIdx) => {
  if (comp.options) comp.options.splice(optIdx, 1)
}
...
const submit = () => {
  ...
  router.put(route('hr.tasks.update', props.task.id), form, {
